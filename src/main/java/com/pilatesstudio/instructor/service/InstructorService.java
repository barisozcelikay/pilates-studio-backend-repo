...
public class InstructorService {
 ...
 @Transactional public InstructorDto create(InstructorDto dto){Instructor instructor=new Instructor();instructor.setAccountId(dto.getAccountId());apply(dto,instructor);return instructorMapper.toDto(instructorRepository.save(instructor));}
 @Transactional public InstructorDto update(InstructorDto dto){if(dto.getId()==null)throw new BusinessException("Güncellenecek eğitmen belirtilmelidir"); Instructor instructor=find(dto.getId());apply(dto,instructor);return instructorMapper.toDto(instructorRepository.save(instructor));}
 ...
 private void apply(InstructorDto dto,Instructor value){value.setSpecialty(dto.getSpecialty());value.setBiography(dto.getBiography());}
 @Transactional public void createForAccount(Long accountId){Instructor instructor=new Instructor();instructor.setAccountId(accountId);instructorRepository.save(instructor);}
}
